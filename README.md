...
**Prototype methods and optimization.** MethodProxies ships with a small set of precompiled trap *prototypes* — one per method arity. Installing a proxy is therefore not a recompilation: the framework simply copies the prototype matching the target method's arity, patches three literals (the handler, the selector of the hidden original method, and the unwind deactivator), and inserts the copy into the method dictionary. Because the trap is a regular `CompiledMethod`, it is JIT-compiled like any other method and the forwarding send to the original is resolved by the VM's polymorphic inline caches, which keeps the per-call overhead low. Stack unwinds are handled directly through the VM's unwind mechanism, **without allocating block closures**, giving significantly better performance than approaches based on `ensure:`.

The trap also tracks a meta-level state on the active process. Whenever a hook executes, the process is marked as *meta*. While in this state, any instrumented method called from within the handler short-circuits to the original behavior, which avoids the infinite-recursion problem that affects simpler approaches.
